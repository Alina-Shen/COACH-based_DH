...
from revwb97m2.step14_recovery_v2 import scalar_values, legacy_pt2, fixed_energy
from revwb97m2.scripts.run_step13_fresh_species import run_qchem

ROOT = Path(__file__).resolve().parents[1]
CONTRACT = ROOT / 'manifests/reaction_features/step14_recovery_v2.json'
...
def main():
    ...
    for name in (['fixed'] if one else ['unit', 'fixed']):
        ...
        if name == 'unit':
            ...
            fixed = fixed_energy(result_text, values['short_range_hf_hartree'])
