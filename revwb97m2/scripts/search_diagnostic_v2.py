...
c=old.c
ROOT=c.ROOT
PLAN=ROOT/'revwb97m2/manifests/search_diagnostic_v2/plan.json'
...
def parameter_value(value):
    """Preserve finite values; encode unlimited parameters without invalid JSON."""
    if isinstance(value, (int, float)) and not np.isfinite(value):
        return c.d.numeric(value)
    return value
...
def solve(folder,case,arrays,settings,start,env):
    ...
    try:
        ...
        c.write(folder/'model.json',dict(case=case,variables=m.NumVars,constraints=m.NumConstrs,
            binaries=m.NumBinVars,version=list(gp.gurobi.version()),parameters={k:parameter_value(m.getParamInfo(k)[2]) for k in keys}))
