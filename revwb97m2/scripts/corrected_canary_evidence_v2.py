"""Version2 evidence adapter: explicit additive dependency policy, unchanged raw audits."""
...
def validate_registry(registry_path=REGISTRY):
    ...
    plan, settings = compat.load_native_plan(checkpoint.PLAN)
...
def components(root, case, settings):
    ...
    inputs = ecp.stage_inputs(source, settings, case['basis_bridge'])
...
def load_large():
    contract = v.read(large.CONTRACT)
    ...
    v.require(report['passed'] and report['original_plan_sha256'] == v.digest(checkpoint.PLAN) and
...
def validate_large(name):
    contract, plan, settings = load_large()
    v.require(name in v.NAMES[5:], 'wrong species')
    case = next(c for c in plan['cases'] if c['species'] == name)
    dest = Path(contract['output_root'])/name
    record = v.read(dest/'publication.json')
    v.require(record['contract_sha256'] == v.digest(large.CONTRACT) and
    ...
    report, vector, fixed, differences = checkpoint.audit_case(plan, settings, case)
    checkpoint.compare_vectors(np.load(dest/'feature_vector_292.npy'), vector)
