"""Generic fixed-orbital generation/reuse; frozen, gated, non-submitting CLI.
...
LAUNCHER = ROOT/'slurm/run_training_features_v1.sh'
...
def validate_reuse(ref, name):
    """Read-only, full native readback of an explicitly pinned completed source."""
    native.require(native.digest(ref['plan']) == ref['plan_sha256'], 'reuse plan changed')
    if ref['kind'] == 'canary':
        report = audit_canary(Path(ref['plan']), name)
        source = Path(native.read(ref['plan'])['output_root'])/name
        if ref.get('recovery'):
            recovery = Path(ref['recovery'])
            saved = native.read(recovery/'recovery.json')
            native.require((recovery/'RECOVERY_COMPLETE').read_text().strip() ==
                           native.digest(recovery/'recovery.json'), 'recovery marker changed')
            for key in ('passed', 'species', 'plan_sha256', 'specification_sha256',
                        'recovery_code_sha256', 'd4_tolerance_hartree', 'artifacts', 'checks'):
                native.require(saved[key] == report[key], 'stale recovery '+key)
        else:
            native.require((source/'CANARY_COMPLETE').is_file(), 'unaccepted canary completion')
        value = np.load(source/'ready/feature_vector_292.npy', allow_pickle=False)
        fixed = native.read(source/'ready/fixed_energy.json')
        differences = {g: np.load(source/f'ready/grid_difference_{g}.npy') for g in native.GRIDS[1:]}
    elif ref['kind'] == 'refresh':
        from revwb97m2.v7_refresh import validate_species
        value, fixed_value, differences = validate_species(Path(ref['plan']), name)
        fixed = {'fixed_energy_hartree': fixed_value}
    else:
        raise ValueError('unsupported reuse kind')
    return value, fixed, differences
...
def freeze(path, names, namespace):
    ...
    for name in names:
        ...
        count, spin = native.electron_count(source.read_text(), bridge)
        native.require((count, spin) == (int(row['electron_count']), int(row['spin'])), 'inventory electron/spin mismatch')
        inputs = native.stage_inputs(source.read_text(), settings, bridge)
    ...
    native.write(path, dict(schema_version=1, purpose='generic_training_feature_batch',
        ...
        selection_source={'path': str(PILOT), 'sha256': native.digest(PILOT)},
        concurrency_proposal=8, resource_review_pending=True,
...
def load(path):
    ...
    native.require(plan['schema_version'] == 1 and plan['purpose'] == 'generic_training_feature_batch', 'wrong plan')
    settings = native.load_fit_settings()
    ...
    for c in plan['cases']:
        ...
        expected = native.stage_inputs(Path(c['authoritative_input']).read_text(), settings, c['basis_bridge'])
...
def release_check(plan_path, release_path):
    ...
    native.require(set(r['canaries']) == set(native.NAMES), 'all seven canaries required')
    for name, ref in r['canaries'].items():
        native.require(ref['kind'] == 'canary' and Path(ref['plan']).resolve() == CANARY.resolve(), 'wrong canary authority')
        validate_reuse(ref, name)
...
def validate(plan_path, name, require_marker=True):
    ...
    vector, fixed, differences, values = native.components(root, case, settings)
...
def main():
    ...
    p.add_argument('action', choices=['freeze-first', 'check', 'run', 'validate', 'audit-reuse'])
    p.add_argument('--plan', type=Path, required=True)
    p.add_argument('--namespace', default='training_first16_v1')
    p.add_argument('--species')
    p.add_argument('--release', type=Path)
    p.add_argument('--reuse-kind', choices=['canary', 'refresh'])
