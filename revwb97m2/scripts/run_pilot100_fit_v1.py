"""Versioned pilot100 solve with telemetry and stronger immutable readback; core model unchanged."""
import argparse
import json
...
def readback(path, identity, arrays, settings):
    ...
    contract=json.loads((path/'contract.json').read_text())
    result=json.loads((path/'result.json').read_text())
    for source,sha in contract['code_sha256'].items():
        if digest(source)!=sha:
            raise ValueError('solver code identity changed')
    for parent,hashes in contract['parent_sha256'].items():
        if any(digest(Path(parent)/name)!=sha for name,sha in hashes.items()):
            raise ValueError('parent lineage changed')
    if result.get('status') not in (2,9) or result.get('solution_count',0)<1:
        raise ValueError('no accepted solver incumbent')
    d=None
    ...
    fresh=gap_report(result['objective'],result['bound'],raw_gap_from_record(result))
    if any(result.get(k)!=v for k,v in fresh.items()):
        raise ValueError('gap readback mismatch')
    for grid in ('99590','75302'):
        maximum=float(np.max(np.abs(arrays['grid_difference_'+grid]@c))*settings.conversion)
        if not np.isclose(maximum,result['grid_max_kcal_mol'][grid],rtol=1e-12,atol=1e-12):
            raise ValueError('grid maximum readback mismatch')
    return c,z
...
def run(args, env=None):
    ...
    try:
        ...
        parameter_names=('Threads','TimeLimit','Seed','FeasibilityTol','IntFeasTol','MIPGap','MIPGapAbs')
        telemetry={'gurobi_version':list(gp.gurobi.version()),
                   'parameters':{k:model.getParamInfo(k)[2] for k in parameter_names}}
        model.optimize()
        telemetry.update(node_count=float(model.NodeCount),status=int(model.Status),
                         solution_count=int(model.SolCount),runtime_seconds=float(model.Runtime))
        (out/'telemetry.json').write_text(strict_dumps(telemetry)+'\n')
