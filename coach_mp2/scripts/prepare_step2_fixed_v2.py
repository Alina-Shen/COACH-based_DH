...
SCRATCH=Path('/clusterfs/mhg-data/yaoshen/scf_read/coach_mp2/step2_fixed_v2')
HEAVY=Path('/clusterfs/mhg-data/yaoshen/coach-based_dh_data/coach_mp2/step2_fixed_v2')
...
def main():
 ...
 for species in ['SIE4x4_h2o','16_C_AE18']:
  ...
  for mode in ['fixed']:
   ...
   inp=ROOT/'inputs/step2_fixed_v2'/f'{species}_{mode}.in';inp.write_text(content)
 ...
 manifest['runtime']=[record(qc/p) for p in ['bin/qchem','build/qcprog.exe','libdft/xcfunctionals.C','scfman/scfman.C']]
 manifest['staged']=a.stage
 if a.stage:
  HEAVY.mkdir(parents=True,exist_ok=True);(HEAVY/'tmp').mkdir(exist_ok=True)
 (ROOT/'manifests/step2_fixed_v2.json').write_text(json.dumps(manifest,indent=2)+'\n')
