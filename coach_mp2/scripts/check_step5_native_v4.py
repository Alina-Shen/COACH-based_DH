...
def close_scf_components(energy):
 energy=dict(energy)
 energy['closure_error_hartree']=energy['component_sum_hartree']-energy['energy_hartree']
 energy['closure_convention']='Seven printed components against native SCF-cycle energy; printed D4 is recorded separately, not added to this SCF closure; VV10 is inside DFT correlation'
 return energy
def main():
 ...
 for case in cases:
  ...
  text=out.read_text();energy=close_scf_components(parse(text));assert 'COACH_MP2: preserving saved MOs; skipping post-SCF orientation' in text
  src=Path('/clusterfs/mhg-data/yaoshen/coach-based_dh_data/coach_mp2/step5_import_v1/species')/name
  receipt=json.loads((src/'import.json').read_text());assert receipt['source']==case['source']
  assert all(receipt['sha256'][f]==h for f,h in case['source_hashes'].items())
  dst=Path(case['scratch']);n,m,*_=struct.unpack('<4i',(src/'819.0').read_bytes());assert n==int(rows[name]['orbital_spherical_aos'])
 ...
 report=dict(native_job_id='25827314',reference='Immutable project imports, verified against both import receipts and native-staging source hashes; external originals are not rehashed after execution',completed=len(results),passed=len(results)==6 and all(r['passed'] for r in results),cases=results,scope='Six fixed-parent read regressions; not MP2 validation or proof of historical basis identity')
 (R/'results/step5_native_v4_validation.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report,indent=2))
