...
constexpr size_t nele_series = integrated_dv_rows;
constexpr size_t nseries = 18;
constexpr size_t nseries_x = 2*nseries;
constexpr size_t nseries_ss = 2*nseries;
constexpr size_t nseries_os = nseries;
...
void eval_exchange_channel(const size_t ngrid, const arma::vec &weights,
    ...
    for(size_t i = 0; i < ngrid; i++) {
        ...
        if((RA > Tol) && (TA > Tol)) {
            ...
            double Fn = 1.0-(2.0/3.0)*a *(a3 - 3.0*a + (2.0*a- a3)*std::exp(-am2)+ 2.0*sqrtPi * std::erf(ainv));
...
void eval_correlation_channels(const size_t ngrid, const arma::vec &weights,
    ...
    assert(RhoA.n_elem == ngrid);
    assert(RhoB.n_elem == ngrid);
    assert(RhoA1.n_rows == ngrid && RhoA1.n_cols == 3);
    assert(RhoB1.n_rows == ngrid && RhoB1.n_cols == 3);
    assert(TauA.n_elem == ngrid);
    assert(TauB.n_elem == ngrid);
    assert(correlation_terms.n_rows == nele_series && correlation_terms.n_cols == 2*(nseries_ss + nseries_os));
    ...
    // Frozen revwb97m2 value. Historical COACH/IDV_print used 0.2.
    double gamma_ss = 0.01;
...
void eval_integrated_dv(const size_t ngrid,
    ...
    assert(integratedDV_batch.n_rows == integrated_dv_rows && integratedDV_batch.n_cols == integrated_dv_cols);
